...
export const createRefreshMyExperiencesAction = (tokenStore: SimulatedTokenStore) => {
  return async (): Promise<RefreshMyExperiencesResult> => {
    try {
      ...
      const response = await fetch("http://10.133.134.146:8080/api/experiences/my-experiences", {
        method: "GET",
        headers: {
