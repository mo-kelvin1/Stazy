...
const WS_URL = "ws://100.66.192.76:8080/ws/chat";
...
const ChatScreen = ({
  ...
  const fetchMessages = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `http://100.66.192.76:8080/api/chats/${recipientEmail}`,
  ...
  const fetchRecipientName = async () => {
    try {
      const token = await tokenStore.getToken();
      const res = await fetch(
        `http://100.66.192.76:8080/api/auth/profile-by-email?email=${encodeURIComponent(
          recipientEmail
        )}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
...
const HostMessagesTab = () => {
  ...
  const fetchThreads = async () => {
    ...
    try {
      const token = await tokenStore.getToken();
      const res = await fetch(`http://100.66.192.76:8080/api/chats/threads`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
        const data = await res.json();
