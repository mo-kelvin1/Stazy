...
const WS_URL = "ws://10.133.134.146:8080/ws/chat";
...
const ChatScreen = ({
  ...
  const fetchMessages = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `http://10.133.134.146:8080/api/chats/${recipientEmail}`,
  ...
  const fetchRecipientName = async () => {
    try {
      const token = await tokenStore.getToken();
      const res = await fetch(
        `http://10.133.134.146:8080/api/auth/profile-by-email?email=${encodeURIComponent(
          recipientEmail
        )}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
...
const MessagesTab = () => {
  ...
  const fetchThreads = async () => {
    ...
    try {
      const token = await tokenStore.getToken();
      const res = await fetch(`http://10.133.134.146:8080/api/chats/threads`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
        const data = await res.json();
